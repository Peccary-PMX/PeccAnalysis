...
test_that("peccary_NCA - Pkanalix absorption (BLQ project without BLQ)", {
  ...
  NCA <- peccary_NCA(dataset = data, timecol =  TIME, obscol = CONC, ID, IVbackextr = F, BLQcol = NULL, outputExpr = F, add_Obs0_Time0 = T)


  # compared to: PKanalix (V2019R2)
  # to_compare <- read.table("D:/PeccAnalysis/tests/dataset_fo_test/PK_analix_absorption.txt", sep = ",", header = T, skip = 1) %>% as_tibble
  to_compare <- read.table("../dataset_fo_test/PK_analix_absorption.txt", sep = ",", header = T, skip = 1) %>% as_tibble
